...
- **2026-09-11 (later same day) — Escalation: `CronCreate` job now dying before its next scheduled firing, not just between firings.** From the 14:43 UTC cycle through at least 15:57 UTC (74+ minutes), every fast-recreate/hourly watchdog found the trading-cycle job already gone and silently recreated it — but no actual "run one full autonomous trading cycle" prompt fired successfully in that entire window (confirmed: no new dated Cycle entry appears in state.md between 14:43 and this note, only watchdog recreation notes). This means the job is now dying *before reaching its next 10-minute scheduled fire time* at least some of the time, not merely between one firing and the next as originally observed (MTBF 13-36 min). Net effect: the loop's actual trading logic (stop-checks, scans, entries) has had no live coverage for over an hour, even though the watchdog layer kept the job's *existence* restored throughout. VRT was checked manually during this window and remains healthy (well clear of stop), so no harm resulted this time, but this is a materially worse failure mode than previously documented and was escalated to the user via PushNotification outside the normal "silent recreation, no notification" convention, since that convention assumes at least intermittent successful firings.
