...
**After creating all four**, verify with `CronList` (Job 1) and `list_triggers` (Jobs 2-4) that all show up, then confirm to the user in a text (from this session, via Inkbox — this session, unlike the Routines, does hold that connector) that the loop is back up.
